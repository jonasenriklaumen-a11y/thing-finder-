...
def test_env_overrides(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.setenv("AQUATICY_DATA_DIR", str(tmp_path / "data"))
    monkeypatch.setenv("AQUATICY_MODEL", "nvidia_nim/meta/llama-3.3-70b-instruct")
    monkeypatch.setenv("AQUATICY_MAX_TOOL_CALLS", "5")
...
def test_write_env_file_preserves_comments(tmp_path: Path) -> None:
    ...
    config.write_env_file({"AQUATICY_MODEL": "neu", "AQUATICY_LANG": "en"}, target)

    content = target.read_text(encoding="utf-8")
    assert "# Kommentar" in content
    assert "AQUATICY_MODEL=neu" in content
    assert "FREMD=behalten" in content
    assert "AQUATICY_LANG=en" in content
    assert "AQUATICY_MODEL=alt" not in content
...
def test_generic_key_covers_unlisted_providers(monkeypatch: pytest.MonkeyPatch) -> None:
    """Jeder LiteLLM-Anbieter laesst sich ueber AQUATICY_API_KEY nutzen."""
    monkeypatch.delenv("AQUATICY_API_KEY", raising=False)
    assert config.api_key_name_for("exotisch/modell") == ""

    monkeypatch.setenv("AQUATICY_API_KEY", "geheim")
    assert config.api_key_name_for("exotisch/modell") == "AQUATICY_API_KEY"
...
def test_broken_model_shows_up_in_missing_requirements(
    ...
    monkeypatch.setenv("AQUATICY_DATA_DIR", str(tmp_path / "data"))
    monkeypatch.setenv("AQUATICY_MODEL", "nvidia/nemotron-3-ultra-550b-a55b")
    problems = config.get_settings().missing_requirements()
...
def test_context_tokens_is_configurable(
    ...
    monkeypatch.setenv("AQUATICY_DATA_DIR", str(tmp_path / "data"))
    monkeypatch.setenv("AQUATICY_MODEL", "ollama_chat/gemma4:12b")
    monkeypatch.setenv("AQUATICY_CONTEXT_TOKENS", "32768")
