...
from aquaticy.memory import Memory, MemoryFull, human_size


@pytest.fixture
def store(tmp_path: Path) -> Memory:
    return Memory(tmp_path / "aquaticy.sqlite3", tmp_path)
...
def test_the_database_holds_no_plain_text(store: Memory, tmp_path: Path) -> None:
    ...
    raw = (tmp_path / "aquaticy.sqlite3").read_bytes()
...
def test_a_stolen_database_stays_unreadable(store: Memory, tmp_path: Path) -> None:
    ...
    thief_dir.mkdir()
    (thief_dir / "aquaticy.sqlite3").write_bytes((tmp_path / "aquaticy.sqlite3").read_bytes())
    thief = Memory(thief_dir / "aquaticy.sqlite3", thief_dir)  # eigener Schluessel
...
def test_the_same_key_reads_the_old_notes(tmp_path: Path) -> None:
    ...
    first = Memory(tmp_path / "aquaticy.sqlite3", tmp_path)
    first.remember("bleibt lesbar", topic="Probe")
    again = Memory(tmp_path / "aquaticy.sqlite3", tmp_path)
...
def test_a_passphrase_needs_no_key_file(tmp_path: Path) -> None:
    ...
    store = Memory(tmp_path / "aquaticy.sqlite3", tmp_path, passphrase="ein gutes Passwort")
    store.remember("nur mit Passwort lesbar")
    assert not (tmp_path / mem.KEY_FILE).exists()

    same = Memory(tmp_path / "aquaticy.sqlite3", tmp_path, passphrase="ein gutes Passwort")
    assert same.recall("Passwort")[0].text == "nur mit Passwort lesbar"

    wrong = Memory(tmp_path / "aquaticy.sqlite3", tmp_path, passphrase="falsches Passwort")
...
def test_plain_text_from_older_versions_still_reads(store: Memory, tmp_path: Path) -> None:
    ...
    with sqlite3.connect(tmp_path / "aquaticy.sqlite3") as conn:
